...
const AddLocation = async (req, res) => {
  ...
  const { user_id, coordinates, type, category, tag, description, place_name } = req.body;
//   console.log(req.files)
  // const new_img_url = img_url.replace(/\\/g, '//');
  const img_url = 'C://Users//pranjal//Downloads//images.png';
  console.log(img_url);
...
const LikeLocation = async (req, res) => {
  ...
  const userHasVotedAlready = await Question.findOne({
    question_id: question_id,
    "likes_by.liked_by_id": liked_by_id,
  });
  if (userHasVotedAlready) {
    console.log(userHasVotedAlready);
    try {
      const addvote = await Location.findOneAndUpdate(
        { location_id: location_id, "likes_by.liked_by_id": liked_by_id },
        { $set: { "likes_by.$.value": 1 } }
      );
      console.log(addvote);
      const addtotalvote = await Location.findOneAndUpdate(
    ...
    try {
      ...
      const addvote = await Location.findOneAndUpdate(
        { location_id: location_id },
        { $push: { likes_by: { liked_by_id: liked_by_id, value: value } } }
      );
      const addtotalvote = await Location.findOneAndUpdate(
        { location_id: location_id },
        { $inc: { likes: 1 } }
...
const RemoveLikeLocation = async (req, res) => {
  const { location_id, liked_by_id } = req.body;
  try {
    const removevote = await Location.findOneAndUpdate(
      { location_id: location_id },
      { $pull: { likes_by: { liked_by_id: liked_by_id } } }
    );
    const addtotalvote = await Location.findOneAndUpdate(
      { location_id: location_id },
      { $inc: { likes: -1 } }
...
const DislikeLocation = async (req, res) => {
  const { location_id, liked_by_id } = req.body;
  const userHasVotedAlready = await Location.findOne({
    location_id: location_id,
    "likes_by.liked_by_id": liked_by_id,
  });
  if (userHasVotedAlready) {
